...
async def test_select_player(lead_player_game: GameFlowService, lead_conn: MockWebSocket):
    ...
    with pytest.raises(GameFlowError):
        await lead_player_game._handle_select_player(player.user_id)
    ...
    with pytest.raises(GameFlowError):
        await lead_player_game._handle_select_player(another_player.user_id)
...
async def test_only_lead_select_player(first_player_game: GameFlowService):
    ...
    with pytest.raises(GameFlowError):
        await first_player_game._handle_select_player(first_player_game.player.user_id)
